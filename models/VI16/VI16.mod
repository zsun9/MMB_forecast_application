...
% note 2: Eq 18b is shifted on period forward to guarantee determinacy. In the original code only the 
...
hours_sw07_obs ffr_obs gdpdef_obs gdp_rgd_obs c_rgd_obs ifi_rgd wage_rgd_obs; 
...
G_Y         = 0.20;        %g/Y
phi         = 1.81;        %Inverse Frisch elasticity of L supply
h           = 0.44;        %habit parameter
...
THETA       = 1.34;        % fixed costs in production

sig_p       = 0.89;        %Calvo parameter for retail firms
sig_pi      = 0.36;        % indexation to past inflation
sig_w       = 0.84;        %Calvo parameter for unions
sig_wi      = 0.34;        % indexation to past wage inflation
zeta        = 0.82;        %EDCU
ksi         = 4.27;        %2nd derivative of Inv adjustment cost

rho_PI      = 1.89;        %Response to inflation in Taylor rule
rho_Y       = 0.09;        %Response to output gap in Taylor rule
rho_DY      = 0.20;         %Response to change in output gap in Taylor rule
rho_r       = 0.85;        %Interest rate smoothing in Taylor rule
trend       = 0.32;
picbar      = 0.64; 

rho_A       = 0.94;        %Persistency of technological shock
rho_X       = 0.99;        %Persistency of shock to K
rho_G       = 0.96;   
rho_W       = 0.20;
rho_P       = 0.31;     
rho_k       = 0.99;
...
gdp_rgd_obs     = y-y(-1)+trend;
c_rgd_obs     = c-c(-1)+trend;
ifi_rgd_obs   = i-i(-1)+trend;
wage_rgd_obs     = w-w(-1)+trend;
gdpdef_obs = pi + picbar;
ffr_obs   = r + conster;
hours_sw07_obs = l + constelab;
...
var e_k; stderr 0.6017;
var e_r; stderr 0.2397;
var e_p; stderr 0.1655;
var e_w; stderr 0.297;
end;
...
%stoch_simul(irf=20) y i pi c ffr_obs q n ext_pr;