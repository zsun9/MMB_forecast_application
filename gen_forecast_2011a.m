...
for model = p.models
    ...
    if ~isempty(strfind(model{1},'GLP'))     %contains(model, 'GLP')
        continue
    end
    
    t.path.model = strcat(p.path.models, '\\' , model);

    if isequal(exist(t.path.model{1}, 'dir'),7) %%isfolder(t.path.model) 
        ...
        for vintage = p.vintages
            for scenario = p.scenarios
                ...
                cd(p.path.root);
                t.name.workingpath = strcat(model , '_' , strrep(vintage, '-', '') , '_' , scenario);
                t.path.working = strcat(p.path.estimations , '\\' , t.name.workingpath);
                
                if isequal(exist(t.path.working{1}, 'dir'),7)
                    ...
                    warning(sprintf('Folder %s does not exist, will be created.', t.name.workingpath{1}));
                    copyfile(t.path.model{1}, t.path.working{1});
                end
                
                % retreive Dynare scripts and varobs
                cd(t.path.working{1});
                t.script.modfile = fileread(t.name.modfile{1});
...
%                 end
                ...
                if isequal(exist(t.path.data{1},'file'),2) %isfile(t.path.data{1})
                    copyfile(t.path.data{1}, t.path.working{1});
                else
                ...
                t.xlsRange = strcat('B1:', p.ExcelColumnUntil{1}, num2str(p.nobs + (~any(strcmp(scenario{1},'s1')))));
                ...
                t.script.estimation = ['\nestimation(nodisplay, smoother, order=1, prefilter=0, mode_check, bayesian_irf, '  ...
                    sprintf('datafile=%s, ', t.dataFile{1})  ...
                    sprintf('xls_sheet=%s, ', scenario{1})  ...
               ...
               [t.data.table,t.data.name,~] = xlsread(t.path.data{1}, scenario{1});
                ...
                t.name.modefile = [model{1}  '_mode.mat'];  
                ...
                if any(strcmp(model{1},'QPM08')); %model == "QPM08"  
                    if any(strcmp(scenario{1},'s1')); %scenario == "s1" % in scenario 1, first GDP forecast is nowcast
                ...
                JSONFile = fopen(strcat(model{1},'.json'),'w'); %fopen(char(t.name.modfile),'w')
