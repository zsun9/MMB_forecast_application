import pathlib
import pandas as pd
import get_vintage_data, sample_ends_at_vintagedate

infoSpf = pd.read_csv(pathlib.Path('../data')  / 'spf_dates.csv', encoding='utf-8', index_col=0)

for deadline in infoSpf['deadlines']:

    if pd.to_datetime(deadline).year in {2009}: # 2001, 2008, 2020
        quarterStart, quarterEnd = sample_ends_at_vintagedate.main(vintageDate=deadline, sampleSize=100)
