...
% Stefania Villa
% Model SWGK for the EA

var emp robs piobs dy dc dinv dw ewma epinfma y inv ext_pr pinf c lev q Lambda ell eta v rk invss z s mu n n_e lend u    
...
FCP        = 1.38;           % fixed costs in production
...
#pic_ss     = 1+picbar/100;
#HBAR   = H;%H/GAMMAT
#BETABAR=CBETA;%CBETA*GAMMAT^(-SIGMAC)
#R_SS = 1/CBETA;
#R_K_BAR = R_K_SS;%/GAMMAT
#spread=R_K_SS/R_SS;
...
zf = (1/Z_SS)*((LEV_SS*R_K_SS)*rkf+(R_SS*(1-LEV_SS))*rf(-1)+
    ((R_K_SS-R_SS)*LEV_SS)*levf(-1));          
xf = levf+zf-levf(-1);                            
levf = etaf(+1)+(NU_SS/(LAMBDA-NU_SS))*vf(+1); %if I change the timing BK is not satisfied
n_ef = nf(-1)+zf;                         
lendf(+1)+qf=nf+levf; 
...
%CAPITAL PRODUCERS
invf = (1/(1+BETABAR))* (invf(-1) + BETABAR*invf(+1)+(1/(S_PRIMEP))*(qf +invss))  ;
%lendf  =  ((1-DELTA_SS)/GAMMAT)*(lendf(-1)+kq)+I_OVER_K*invf + I_OVER_K*GAMMAT^2*S_PRIMEP*invss ;
lendf  =((1-DELTA_SS))*(lendf(-1)+kq)+DELTA_SS*(invf)+ I_OVER_K*S_PRIMEP*invss  ; %+ invss
...
Lambda = mu-mu(-1);                        
Lambda(+1)+ r - pinf(+1) = 0;                     
w   =  (1/(1+BETABAR))*w(-1) +(BETABAR/(1+BETABAR))*w(+1)+(SIGMA_WI/(1+BETABAR))
*pinf(-1)-(1+ BETABAR*SIGMA_WI)/(1+BETABAR)*pinf+(BETABAR)/(1+BETABAR)*pinf(+1)
+ (1-SIGMA_W)*((1-BETABAR*SIGMA_W) *(1/(1+BETABAR))
...
z = (1/Z_SS)*((LEV_SS*R_K_SS )*rk+(R_SS*(1-LEV_SS  ))*(r(-1)-pinf)+
    ((R_K_SS-R_SS)*LEV_SS)*lev(-1));        
x = lev+z-lev(-1);                          
...
zk=w+ell-lend(-1)-u;
%CAPITAL PRODUCERS
inv = (1/(1+BETABAR))* (inv(-1) + BETABAR *inv(+1)+(1/(S_PRIMEP))*(q+invss) )  ;
lend=  ((1-DELTA_SS))*(lend(-1)+kq)+DELTA_SS*(inv)+ I_OVER_K*S_PRIMEP*invss ;%+invss)  ;  
% RESOURCE CONSTRAINTS
y=(C_OVER_Y)*c+(I_OVER_Y)*inv+G_Y*G+K_OVER_Y*ZK_SS*u;      
%MONETARY AUTHORITY
pinf =  (1/(1+BETABAR *SIGMA_PI)) * (BETABAR *pinf(+1) +SIGMA_PI*pinf(-1)+((1-SIGMAP)*(
1-BETABAR*SIGMAP)/SIGMAP)*(ALPHA*zk-a+(1-ALPHA)*(w) ) ) + spinf ; 
...
piobs = 1*(pinf) + picbar;
robs =    1*(r) + conster;
emp=(1/(1+CBETA))*emp(-1)+(CBETA/(1+CBETA))*emp(+1)+((1-CBETA*SIGMA_EM)*
   (1-SIGMA_EM)/((1+CBETA)*SIGMA_EM))*(ell-emp);       
...
varobs dy dc dinv emp piobs dw robs;

estimated_params;

stderr e_a,   0.9885 ,0.01,25,INV_GAMMA_PDF,0.1,2;
stderr e_x,   2.2978 ,0.01,50,INV_GAMMA_PDF,0.1,2;
stderr e_i,   0.1120 ,0.01,25,INV_GAMMA_PDF,0.1,2;
stderr e_kq,  0.1338 ,0.025,25,INV_GAMMA_PDF,0.1,2;
stderr e_g,   1.4860 ,0.01,50,INV_GAMMA_PDF,0.1,2;
stderr epinf, 0.0652 ,0.01,25,INV_GAMMA_PDF,0.1,2;
stderr ew,    0.1180 ,0.01,25,INV_GAMMA_PDF,0.1,2;
%%%%%%%%%%%%%%%%%%%%%%%%%
RHO_A,         0.8884 ,.01,.99999,BETA_PDF,0.5,0.20;
RHO_RP,        0.9802 ,.01,.99999,BETA_PDF,0.5,0.20;
RHO_G,         0.9424 ,.01,.9999999,BETA_PDF,0.5,0.20;
RHO_X,         0.9729 ,.01,.9999999,BETA_PDF,0.5,0.20;
RHO_MS,        0.2132 ,-0.02,.9999,BETA_PDF,0.5,0.20;
RHO_P,         0.8475 ,.01,.99999,BETA_PDF,0.5,0.20;
RHO_W,         0.5915 ,.001,.9999,BETA_PDF,0.5,0.20;
S_PRIMEP,      3.9648 ,0.0001,15,NORMAL_PDF,4,1.5;
H,             0.6479 ,0.1,0.99,BETA_PDF,0.7,0.1;
SIGMA_W,       0.7648 ,0.25,0.99,BETA_PDF,0.75,0.05;
SIGMAP,        0.8149 ,0.5,0.97,BETA_PDF,0.75,0.05;
SIGMA_EM,      0.8021 ,0.1,0.95, BETA_PDF,0.5,0.15;
FCP,           1.3562 ,1.0,3,NORMAL_PDF,1.45,0.125;
PHI,           1.3973 , 0.1, 3.5,gamma_pdf, 0.33,  0.25;
SIGMA_WI,      0.4217 ,0.01,0.99,BETA_PDF,0.5,0.15;
SIGMA_PI,      0.1257 ,0.01,0.99,BETA_PDF,0.5,0.15;
ZETA,          0.9554 ,0.0001,1,BETA_PDF,0.25,0.15;
IOTA_PI,       1.7447 , 1.0,3,NORMAL_PDF,1.7,0.15;
RHO_I,         0.8895 ,0.35,0.975,BETA_PDF,0.75,0.10;
IOTA_Y,        0.0709 ,-0.065 ,0.5,GAMMA_PDF,.125,0.05;
%IOTA_DPI,      0.0931 , -0.145,0.6, normal_pdf, 0.3,0.1;
IOTA_DY,       0.0682 ,-0.1,0.5,NORMAL_PDF,0.0625,0.05;
%%%%%%%%%%%%%%%%%%%%%
picbar,       0.6313 ,0.1,2.0,GAMMA_PDF,0.625,0.1;
ctrend,       0.1779 ,0.001,0.8,NORMAL_PDF,0.4,0.10;
...
%,,,,bayesian_irf
% estimation(datafile=data_villaea,mode_compute=0,mode_file=swgkear4_mode,first_obs=53,nobs = 103, presample=0,
% lik_init=2,prefilter=0,conf_sig=0.95,mh_replic=0,load_mh_file,mh_nblocks=2,mh_jscale=0.35,
% mh_drop=0.25,moments_varendo) y inv c pinf robs n ext_pr ;

%it gives IRF and variance decomposition,,conditional_variance_decomposition=[1 4 8 40 80]
stoch_simul(irf=40,nograph)y inv pinf ext_pr;