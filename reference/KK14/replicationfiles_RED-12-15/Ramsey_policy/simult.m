...
for i=1:replic
    ...
    disp(' ');
    disp('Please name (incl. file extension) the file you want use for simulation')
    filename=input('otherwise new shocks will be drawn: ','s'); 
%filename='results_oct2012_5v_2lag'
%load(filename);
    
    
    if exist(filename)==2
       load(filename);
       oo_.exo_simul(:,1:7)=exo_simul(2001:end,:);
       
        %oo_.exo_simul(:,1:7)=exo_simul(51:end,:);
    else
        
        disp('I cannot found the file, so I will take new draws for shocks');
         oo_.exo_simul(:,i_exo_var) = randn(options_.periods,nxs)*chol_S;
    end
