...
estimated_params;
sigma_c, 1.64, gamma_pdf, 1.75, 0.5;
sigma_l,4, gamma_pdf, 2, 1;
h,  0.45, beta_pdf, 0.5, 0.15;
gamma_p,.64, beta_pdf, 0.5, 0.1;
gamma_w,.65,beta_pdf, 0.5, 0.1;
//xi_p,.3, beta_pdf, 0.5, 0.1;
//xi_w,.3,beta_pdf, 0.5, 0.1;
nu, 6, gamma_pdf,4, .75;
sigma_u, 3.1, gamma_pdf, 2, 0.5;

rho_R,0.8, beta_pdf, 0.8, 0.1;
rho_pi,1.8 , gamma_pdf, 1.7,0.1;
rho_y,0.12, gamma_pdf, 0.125, 0.05;

rho_eps_i,0.4923, beta_pdf, 0.85, 0.1;
rho_eps_z,0.92, beta_pdf, 0.85, 0.1;
rho_eps_q,0.8335, beta_pdf, 0.85, 0.1;
rho_tr,0.76, beta_pdf, 0.85, 0.1;
rho_cg,0.88377, beta_pdf, 0.85, 0.1;
rho_P,0.2, beta_pdf, 0.85, 0.1;
//rho_L,0.2, beta_pdf, 0.85, 0.1;

rho_w, 0.8551, beta_pdf, 0.85,  0.1;
etaWb, 0.1128, normal_pdf, 0, .5;
etaWy, 0.1458, normal_pdf, 0, .5;

rho_k,  0.8676, beta_pdf, 0.85,  0.1;
etaKb,0.4079, normal_pdf, 0, .5;
etaKy,.26, normal_pdf, 0, .5;


stderr e_i,0.0334, inv_gamma_pdf, 0.01, 4;
stderr e_z,0.0068, inv_gamma_pdf, 0.01, 4;
stderr e_q,0.0224, inv_gamma_pdf, 0.01, 4;
stderr eps_m,0.0016, inv_gamma_pdf, 0.01, 4;
stderr eps_tauw,0.0216, inv_gamma_pdf, 0.01, 4;
stderr eps_tauk,0.0406, inv_gamma_pdf, 0.01, 4;
stderr eps_cg,0.0337, inv_gamma_pdf, 0.01, 4;
stderr eps_tr,0.0612, inv_gamma_pdf, 0.01, 4;
stderr e_P,0.0017, inv_gamma_pdf, 0.01, 4;
stderr e_L,0.0017, inv_gamma_pdf, 0.01, 4;
stderr eps_y,0.0017, inv_gamma_pdf, 0.01, 4;

stderr tax_obs,0.0056, inv_gamma_pdf, 0.01, 4;
end;

varobs  y_obs,c_obs, I_obs, inf_p, R, tau_w, tau_k, b_obs, lp, w_obs, g_obs,tax_obs;
...
//,load_mh_file
,mode_check
,mode_file=model_baseline_mode_backup
//,bayesian_irf, irf=40
//,tex
