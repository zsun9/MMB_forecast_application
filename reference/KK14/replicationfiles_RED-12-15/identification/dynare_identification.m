...
%Added October 2010
n_var_check=size(bayestopt_.mf2,2); 
params_save=zeros(nparam, SampleSize);
moments_save=zeros(n_var_check^2+n_var_check^2-n_var_check*(n_var_check-1)/2, SampleSize);
JJ_save=zeros(n_var_check^2+n_var_check^2-n_var_check*(n_var_check-1)/2, nparam, SampleSize);
        ...
        if info(1)==0,
            ...
            if iteration,
                ...
                %% added
                 ...
                 JJ_save(:,:,iteration)=JJ(indJJ,:);
                params_save(:,iteration)=params';
                
                vcv_draw=dlyap(A, B*M_.Sigma_e'*B');
                auto_1=A*vcv_draw;
                
                moments_save(1:n_var_check^2-n_var_check*(n_var_check-1)/2,iteration)=dyn_vech(vcv_draw(bayestopt_.mf2,bayestopt_.mf2));
                moments_save(n_var_check^2-n_var_check*(n_var_check-1)/2+1:end,iteration)=vec(auto_1(bayestopt_.mf2,bayestopt_.mf2));
                %%
...
save did_JJ_myself moments_save JJ_save params_save 
