function [Gamma_y,stationary_vars] = th_autocovariances(dr,ivar,M_,options_,nodecomposition, bc_freq)
...
if options_.hp_filter == 0
    ...
    if ~nodecomposition && M_.exo_nbr > 1 && size(stationary_vars, 1) > 0
        ...
        warning(['VARIANCE DECOMPOSITION IS COMPUTED AT BUSINESS CYCLE FREQUENCIES'])
            ...
            G    = A;
            HH   = dr.ghu;
            SDX  = M_.Sigma_e+1e-14*eye(M_.exo_nbr);
            
            zmat = zeros(length(iky),size(ghx,1));
            zmat(:,iky) = eye(length(iky));
            
            out=vardecom_spec_Andrea(G,A,ghx,ghu1,ghu,HH,SDX,zmat,freq(2:end-1),[freq(2) - freq(1),freq(end)-freq(end-1)],[],[]);
            
            Gamma_y{nar+2} = real(out.vfbdecz);
            %Gamma_y{nar+2} = real(out.specdecz);
            %Gamma_y{nar+2}
        else
