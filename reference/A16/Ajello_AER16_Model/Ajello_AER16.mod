...
global save_results_dsge_likelihood M_
...
varexo  eps_z  eps_g  eps_i  eps_tau eps_tau_trans eps_phi eps_beta  eps_p  eps_w  eps_meas  eps_meas_sp eps_sg;
...
varobs d_GDP d_Ihat d_chat dlnwhat pi_ob_t i_ob_t l_ob_t Spread_ob_t FGS_obs;
...
sg_ss       ,                0.06       , 0, 1, gamma_pdf     ,  0.40, 0.20;       
...
if optim_ind == 0
    ...
    % Run the model using calibrated parameters

    steady;
    stoch_simul(periods=0, IRF=0, order = 1, conditional_variance_decomposition=[6])d_GDP, d_Ihat, d_chat, dlnwhat, pi_ob_t, i_ob_t , l_ob_t, Spread_ob_t, FGS_obs;
    
elseif optim_ind == 1

    % Model Estimation - part 1
    % Find the posterior mode using several rounds of optimizers (e.g., mode_compute = 4 for csminwel, mode_compute = 5 for newrat, mode_compute = 7 for fminsearch).
    % The code for each optimizer should look like:

    options_.optim_opt = '''MaxIter'', 1000';
    
    estimation(lik_init = 1, order = 1, datafile = MYDATA_baseline, mode_file = 'Ajello_AER16_mode.mat',
    first_obs = 1, plot_priors  = 0, mode_compute = 7, irf = 30, mh_replic = 0) d_GDP, d_Ihat, d_chat, dlnwhat, pi_ob_t, i_ob_t , l_ob_t, Spread_ob_t, FGS_obs;
    
    % Once you have found the posterior mode you can use the parameter values to compute theoretical moments,
    % simulate model and/or plot impulse responses:

    steady;
    stoch_simul(periods=0, IRF=0, order = 1) d_GDP, d_Ihat, d_chat, dlnwhat, pi_ob_t, i_ob_t , l_ob_t, Spread_ob_t, FGS_obs;
    
    % You can also obtain decomposition of observables into smoothed shocks at the posterior mode:

    shock_decomposition(datafile = MYDATA_baseline, parameter_set = posterior_mode) d_GDP, d_Ihat, d_chat, ygap_t;
   
elseif optim_ind == 2

    % Model estimation - part 2
    % MCMC exploration of posterior surface
    % The exploration starts from the posterior mode and from the hessian matrix derived from "Model estimation - part 1"
    % Alternatively the model hessian can be adjusted and
    
    set_dynare_seed('clock');
    options_.sub_draws = 1000;
    estimation(lik_init = 1, mode_file = 'Ajello_AER16_mode.mat', 
    datafile = MYDATA_baseline, first_obs = 1,
    plot_priors  = 0, mode_compute = 0, ar = 3,
    irf = 30, bayesian_irf, mh_replic =100000, mh_nblocks = 3,
    mh_drop = .2, mh_jscale = .40,
    moments_varendo) d_GDP, d_Ihat, d_chat, dlnwhat, pi_ob_t, i_ob_t , l_ob_t, Spread_ob_t, FGS_obs, GDP_t, Ihat, chat, what_t, z_t;
...
filename = ['Ajello_AER16_optim_ind_' int2str(optim_ind) '_' datestr(now,'yyyymmddTHHMMSS')]; 
