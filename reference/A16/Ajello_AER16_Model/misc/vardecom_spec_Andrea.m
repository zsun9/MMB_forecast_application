...
pshat=disclyap_fast_Andrea(G1,MM);
...
for jj=1:nf;
    ...
    out.specz(:,:,jj)= zmat*(ghx*(temp_spec)*ghx' + ghu*(cs'*cs)*ghu')*zmat';
...
for hh=1:nx
    ...
    for jj=1:nf
        ...
        out.specdecz(:,hh,jj)=diag(zmat*(ghx*(temp_spec)*ghx' + ghu*(cs(hh,:)'*cs(hh,:))*ghu')*zmat')./(diag(out.specz(:,:,jj)));
        out.vfbdecz(:,hh)=(out.vfbdecz(:,hh))+2*(diag(zmat*(ghx*(temp_spec)*ghx' + ghu*(cs(hh,:)'*cs(hh,:))*ghu')*zmat')*binw(jj));
    end
end
