...
[para_standard_deviations,para_correlation_matrix]=Cov2Corr(inv(hh));  
...
if  ~isnan(ExtremeCorrBound)
    ...
    for i=1:length(RowIndex)
        ExtremeCorrParams{i,1}=char(bayestopt_.name(RowIndex(i),:));
        ExtremeCorrParams{i,2}=char(bayestopt_.name(ColIndex(i),:));
