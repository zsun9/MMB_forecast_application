...
//                                                                      //
//Parameters here are set such that the simple wage rule model is active//
//That is, the AOB sharing rule is replaced by the simple wage rule as  //
...
//R21 Sharing rule
//see below for simple wage rule
...
//Simple Wage Rule
wR            =  STEADY_STATE(wR)   
+ bet1*(wR(-1) -  STEADY_STATE(wR))  
+ bet2*(lR(-1) -  STEADY_STATE(lR)); 

wF             =  STEADY_STATE(wF)   
+ bet1*(wF(-1) -  STEADY_STATE(wF))  
+ bet2*(lF(-1) -  STEADY_STATE(lF)) 
+ bet3*(muzF   -  STEADY_STATE(muzF))               
+ bet4*(mupsiF -  STEADY_STATE(mupsiF)); 
...
bet1,,,,beta_pdf,0.75,.1;
bet2,.1,,,normal_pdf,0,1;
bet3,,,,normal_pdf,0,1;
bet4,,,,normal_pdf,0,1;
