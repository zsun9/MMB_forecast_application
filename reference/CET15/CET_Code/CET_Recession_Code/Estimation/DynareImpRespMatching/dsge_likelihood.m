...
%&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
global M_ options_ oo_ horizon logdetVhat Vhat inv_Vhat psihat data_varnames mod_var_list mod_shock_list doMonetaryShockonly
...
[i_var_exo,nvar_exo] = varlist_indices(mod_shock_list,M_.exo_names);

[oo_.dr.ys,M.params,info] = evaluate_steady_state(oo_.steady_state,M_,options_,oo_,0);
...
for jj=i_var_exo,    
    ...
    %mod_var_list=[{'GDPAGG'} {'piAGG'} {'RAGG'} {'lAGG'} {'wAGG'}  {'cAGG'}  {'iAGG'} {'pinvestAGG'}  {'unempAGG'} {'vTotAGG'} {'LAGG'} {'fAGG'} {'ukAGG'} {'muF'} {'mupsiF'}];
    ...
    if jj==1, %monetary shock
        ...
        psitheta_mon(2:end,1)=yFF(2:end,1);%GDP
        psitheta_mon(2:end,2)=yFF(2:end,2); %infl
        psitheta_mon(1:end,3)=yFF(1:end,3); %FF
        psitheta_mon(2:end,4)=yFF(2:end,4); %capacity utilization
        psitheta_mon(2:end,5)=yFF(2:end,5); %total hours
        psitheta_mon(2:end,6)=yFF(2:end,6); %wage
        psitheta_mon(2:end,7)=yFF(2:end,7); %cons
        psitheta_mon(2:end,8)=yFF(2:end,8); %inv
        psitheta_mon(2:end,9)=cumsum(yFF(2:end,9)); %rel.price.invest
        psitheta_mon(2:end,10)=yFF(2:end,10)*M_.params(loc(M_.param_names,'u')); %unemp
        psitheta_mon(2:end,11)=yFF(2:end,11);%vacancies
        psitheta_mon(2:end,12)=yFF(2:end,12); %total hours
        psitheta_mon(2:end,13)=yFF(2:end,13)*M_.params(loc(M_.param_names,'f'));%finding rate
    ...
    if jj==2, %neutral tech shock
        yFF=100*y(i_var,1:horizon)';
        mulev_vec=cumsum(yFF(:,end-1)); %get levels of composite tech.
        mupsilev_vec=cumsum(yFF(:,end)); %get levels of invest. spec. tech.
        
        psitheta_ntech=NaN*zeros(horizon,length(mod_var_list));
        psitheta_ntech(1:end,1)=yFF(1:horizon,1)+mulev_vec(1:horizon);%GDP
        psitheta_ntech(1:end,2)=yFF(1:horizon,2); %infl
        psitheta_ntech(1:end,3)=yFF(1:horizon,3); %FF
        psitheta_ntech(1:end,4)=yFF(1:horizon,4); %capacity utilization
        psitheta_ntech(1:end,5)=yFF(1:horizon,5); %total hours
        psitheta_ntech(1:end,6)=yFF(1:horizon,6)+mulev_vec(1:horizon); %wage
        psitheta_ntech(1:end,7)=yFF(1:horizon,7)+mulev_vec(1:horizon); %cons
        psitheta_ntech(1:end,8)=yFF(1:horizon,8)+mulev_vec(1:horizon)+mupsilev_vec(1:horizon); %inv
        psitheta_ntech(1:end,9)=cumsum(yFF(1:horizon,9)); %rel.price.invest
        psitheta_ntech(1:end,10)=yFF(1:horizon,10)*M_.params(loc(M_.param_names,'u')); %unemp
        psitheta_ntech(1:end,11)=yFF(1:horizon,11);%vacancies
        psitheta_ntech(1:end,12)=yFF(1:horizon,12); %total hours
        psitheta_ntech(1:end,13)=yFF(1:horizon,13)*M_.params(loc(M_.param_names,'f'));%finding rate
    ...
    if jj==3, %invest tech shock
        ...
        psitheta_itech=NaN*zeros(horizon,length(mod_var_list));
        psitheta_itech(1:end,1)=yFF(1:horizon,1)+mulev_vec(1:horizon);%GDP
        psitheta_itech(1:end,2)=yFF(1:horizon,2); %infl
        psitheta_itech(1:end,3)=yFF(1:horizon,3); %FF
        psitheta_itech(1:end,4)=yFF(1:horizon,4); %capacity utilization
        psitheta_itech(1:end,5)=yFF(1:horizon,5); %total hours
        psitheta_itech(1:end,6)=yFF(1:horizon,6)+mulev_vec(1:horizon); %wage
        psitheta_itech(1:end,7)=yFF(1:horizon,7)+mulev_vec(1:horizon); %cons
        psitheta_itech(1:end,8)=yFF(1:horizon,8)+mulev_vec(1:horizon)+mupsilev_vec(1:horizon); %inv
        psitheta_itech(1:end,9)=cumsum(yFF(1:horizon,9)); %rel.price.invest
        psitheta_itech(1:end,10)=yFF(1:horizon,10)*M_.params(loc(M_.param_names,'u')); %unemp
        psitheta_itech(1:end,11)=yFF(1:horizon,11);%vacancies
        psitheta_itech(1:end,12)=yFF(1:horizon,12); %total hours
        psitheta_itech(1:end,13)=yFF(1:horizon,13)*M_.params(loc(M_.param_names,'f'));%finding rate
...
likelihood=(size(psihat,1)/2*log(1/2/pi)-1/2*logdetVhat-1/2*criterion); %in logs
