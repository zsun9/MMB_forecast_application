...
normobs=2007;
trendfrom=1985;%1982;%1950;%1978;%2001.0;
trenduntil=2008.25;
showfrom=1985;%1982;%1950;%1978;
...
plot(tfptime(idxshow:end),BLSpriv(idxshow:end),'b-','LineWidth',3);hold on;
%plot(tfptime(idxshow:end),Fernaldutil(idxshow:end),'b-','LineWidth',3);hold on;
%plot(tfptime(idxshow:end),FernaldutilSolow(idxshow:end),'m-','LineWidth',3);hold on;
...
plot(tfptime(idxshow:end),BLSprivtrend(idxshow:end),'b--','LineWidth',1.5);hold on;
%plot(tfptime(idxshow:end),FernaldutilSolowtrend(idxshow:end),'m--','LineWidth',1.5);hold on;
...
plot(tfptime(idxshow:end),BLSprivtrendforecast(idxshow:end),'b:','LineWidth',1.5);hold on;
%plot(tfptime(idxshow:end),FernaldutilSolowtrendforecast(idxshow:end),'m:','LineWidth',1.5);hold on;
...
axis tight
title('Log Total Factor Productivity (TFP): 1985-2013','FontSize',12);
%legend('Fernald (Raw)','Fernald (Util. Adjusted)','Own (Fernald, Solow Util.)','Penn World Tables',2);
legend('Fernald (Raw)','BLS (Private Business)','Penn World Tables',2);
legend boxoff

text('String',['Notes: Linear trend from 1985Q1-2008Q2 (dashed-dotted). Forecast from 2008Q3 based on linear trend (dotted).'],...
    'Position',[1984.75 4.285 0],'FontSize',9);
...
%plot(tfptime(idxtrenduntil+1:end),100*FernaldutilSolowdev(idxtrenduntil+1:idxtrenduntil+obs-idxtrenduntil),'m-','LineWidth',3);hold on;
...
%plot(tfptime(idxtrenduntil+1:end),0*tfptime(idxtrenduntil+1:end)+.5,'k-','LineWidth',.1);hold on;
%legend('Fernald (Raw)','Fernald (Util. Adjusted)','Own (Fernald, Solow Util.)','Penn World Tables','Our Model',1);
leg=legend('Fernald (Raw)','BLS (Private Business)','Penn World Tables','Our Model',1);
set(leg,'FontSize',10);
legend boxoff
axis tight
axis([2008.5 2013.25 -7 1]);
recession_bars;
plot(tfptime(idxtrenduntil+1:end),100*Fernaldrawdev(idxtrenduntil+1:idxtrenduntil+obs-idxtrenduntil),'g-','LineWidth',3);hold on;
plot(tfptime(idxtrenduntil+1:end),100*BLSprivdev(idxtrenduntil+1:idxtrenduntil+obs-idxtrenduntil),'b-','LineWidth',3);hold on;
%plot(tfptime(idxtrenduntil+1:end),100*FernaldutilSolowdev(idxtrenduntil+1:idxtrenduntil+obs-idxtrenduntil),'m-','LineWidth',3);hold on;
...
%plot(tfptime(idxtrenduntil+1:end),0*tfptime(idxtrenduntil+1:end)+.5,'k-','LineWidth',.1);hold on;
axis([2008.5 2013.25 -7 1]);
time_str={'2008Q3',	  '2009Q3',	  '2010Q3',	  '2011Q3',  '2012Q3', 	'2013Q2',	 };
set(gca,'Xtick',[2008.5 2009.5 2010.5 2011.5 2012.5 2013.25]);
set(gca,'Xticklabel',time_str);
set(gca,'FontSize',12);
title('TFP (% Deviation from Trend)','FontSize',12);

suptitle2('Figure 6: Measures of Total Factor Productivity: 1985-2013')

orient portrait;
set(gcf, 'PaperPositionMode', 'manual');
set(gcf, 'PaperUnits', 'inches');
set(gcf, 'PaperPosition', [.2 1 8 9]);

print -dpdf ModelvsTFPcollectionlong; print -depsc ModelvsTFPcollectionlong; saveas(gcf, 'ModelvsTFPcollectionlong','fig');
