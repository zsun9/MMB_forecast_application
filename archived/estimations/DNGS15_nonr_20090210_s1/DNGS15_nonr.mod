...
%%% observables %%%
    gdp_rgd_obs          $\Delta y^{obs}$    % 28    => GDP growth, observed
    hours_dngs15_obs           $L^{obs}$           % 29    => hours worked, observed
    wage_rgd_obs            $wage^{obs}$        % 30    => real wage growth, observed
    gdpdef_obs          $\pi^{obs}$         % 31    => inflation, observed
    ffr_obs           $R^{obs}$           % 32    => policy rate, observed (shadow rate)
    c_rgd_obs          $\Delta c^{obs}$    % 33    => consumption growth, observed
    ifi_rgd_obs           $\Delta i^{obs}$    % 34    => investment growth, observed
    baag10_obs     		$SPREAD^{obs}$      % 35    => interest rate spread, observed
...
% nominal rigidities
    zeta_p    = 0.10;    % price Calvo prametert
    iota_p    = 0.10;    % price indexation parameter
    zeta_w    = 0.10;    % wage Calvor parameter
    iota_w    = 0.10;    % wage indexation parameter
...
% other endogenous propagagion and steady state parameters
    ...
    Bigphi    = 1.00;    % fixed cost of production
    h         = 0.50;    % habit persistence
    ...
    s2        = 0.75;    % second derivative investment adjustment cost
    sigmac    = 1.50;    % relative risk aversion
    ppsi      = 0.10;    % capital utilization cost
...
%***********************************************/
%%% measurement equations %%%
gdp_rgd_obs       =  ( y_t - y_t(-1) + z_t)  +  100*(exp(zstar)-1);
c_rgd_obs       =  ( c_t - c_t(-1) + z_t)  +  100*(exp(zstar)-1);
ifi_rgd_obs        =  ( i_t - i_t(-1) + z_t)  +  100*(exp(zstar)-1);
wage_rgd_obs         =  (w_t - w_t(-1) + z_t)   +  100*(exp(zstar)-1);
ffr_obs        =  R_t  + Rstarn;
gdpdef_obs       =  pi_t + 100 * (pistar-1);
baag10_obs         =  ( Rktil_t(+1) - R_t ) +  100*log(sprd);
hours_dngs15_obs        =  L_t +  Lmean;
...
% monetary policy parameters
psi1,NORMAL_PDF,1.5,0.25;% monetary policy reaction to inflation   %
rho_r,BETA_PDF,0.75,0.10;% interest rate smoothing parameter
psi2,NORMAL_PDF,0.12,0.05;% monetary policy reaction to the output gap
psi3,NORMAL_PDF,0.12,0.05;% monetary policy reaction to the change in the output gap

% nominal rigidities
%zeta_p,BETA_PDF,0.1,0.02;  % price Calvo prametert
%zeta_w,BETA_PDF,0.1,0.02; % wage Calvo prametert
%iota_w,BETA_PDF,0.1,0.025;% price indexation  %originally fixed 0.5
%iota_p,BETA_PDF,0.1,0.025;% price indexation  %originally fixed 0.5

% financial frictions parameters
zeta_spb,beta_pdf,0.05,0.005;  % elasticity of the spread with respect to leverage
sprd_aux,gamma_pdf,2.00, 0.10;    % steady state spread

% other endogenous propagagion and steady state parameters
alp,NORMAL_PDF,0.3,0.05; % capital elasticity (Cobb-Douglas production function)
Bigphi,NORMAL_PDF,1.25,0.12; % fixed cost of production
h,BETA_PDF,0.7,0.1;% habit persistence
nu_l,NORMAL_PDF,2,0.75;% inverse of the elasticity of labor with respect to the real wage
rstar_aux,GAMMA_PDF,0.25,0.1;% steady state real interest rate
pistar_aux,GAMMA_PDF, 0.75, 0.40; % quarterly steady state inflation rate
zstar_aux,NORMAL_PDF,0.4,0.10; % quarterly steady state growth rate of labor augmenting technology growth
%s2,NORMAL_PDF,1,0.025;% second derivative investment adjustment cost
sigmac,NORMAL_PDF,1.50,0.37 ;% relative risk aversion
%ppsi,BETA_PDF,0.1,0.025;% capital utilization cost
Lmean,NORMAL_PDF,-45,5; % captures the units of measured hours worked.

%% exogenous processes - autocorrelation
rho_z,BETA_PDF,0.5,0.20;% persistence of technology shock process
rho_b,BETA_PDF,0.5,0.20; % persistence of asset shock
rho_g,BETA_PDF,0.5,0.20; % persistence of government spending shock process
rho_mu,BETA_PDF,0.5,0.20;
rho_rm,BETA_PDF,0.5,0.20; % persistence of monetary policy shock process
rho_laf,BETA_PDF,0.5,0.20;% persistence of price mark-up shock process
rho_law,BETA_PDF,0.5,0.20;% persistence of marginal efficiency of investment (MEI) shock process
eta_laf,BETA_PDF,0.5,0.2;% ARMA parameter of price mark-up shock process
eta_law,BETA_PDF,0.5,0.2;% ARMA parameter of wage mark-up shock process
eta_gz,BETA_PDF,0.5,0.2;% elasticity of government spending to technology shock
rho_sigw,beta_pdf,0.75,0.15;  % persistence of the spread shock process

%% exogenous processes - standard deviation
stderr z_sh,INV_GAMMA_PDF,0.1,2;
stderr g_sh,INV_GAMMA_PDF,0.1,2;
stderr b_sh,INV_GAMMA_PDF,0.1,2;
stderr mu_sh,INV_GAMMA_PDF,0.1,2;
stderr laf_sh,INV_GAMMA_PDF,0.1,2;
stderr law_sh,INV_GAMMA_PDF,0.1,2;
stderr sigw_sh,INV_GAMMA_PDF,0.05,4.00; %
stderr rm_sh,INV_GAMMA_PDF,0.1,2;



end;

varobs gdp_rgd_obs hours_dngs15_obs wage_rgd_obs gdpdef_obs ffr_obs c_rgd_obs ifi_rgd_obs baag10_obs;

%varobs delta_y_t_obs delta_c_t_obs delta_i_t_obs wage_t_obs hours_t_obs infl_t_obs int_t_obs baag10_obs;
...
estimation(nodisplay, smoother, order=1, prefilter=0, datafile=data_20090210, xls_sheet=s1, xls_range=B1:BD100, presample=4, mh_replic=500000, mh_nblocks=1, mh_jscale=0.3, mh_drop=0.3, sub_draws=5000, forecast=40, mode_compute=4) gdp_rgd_obs;