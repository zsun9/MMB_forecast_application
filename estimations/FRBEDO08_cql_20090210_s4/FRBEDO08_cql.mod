...
gdp_rgd_obs = (hgdp*exp(eta_xgdp)-1)*100;
gdpdef_obs  = (paipgdp*exp(eta_pgdp)-1)*100;
ffr_obs = (r-1)*100;

cnds_nom_obs = (paipc*ecnn/ecnn(-1)*gammaxc*exp(eta_pecnn) -1)*100;
cd_nom_obs  = (paipk*ecd/ecd(-1)*gammaxk*exp(eta_pecd)-1)*100;
ir_nom_obs   = (paipc*er/er(-1)*gammaxc*exp(eta_per)-1)*100;
inr_nom_obs  = (paipk*enr/enr(-1)*gammaxk*exp(eta_penr)-1)*100;
cnds_def_obs = (paipc*exp(eta_paipc)-1)*100;
cd_def_obs = (paipk*exp(eta_paipk)-1)*100;
wage_rgd_obs    = (paipc/paipgdp*gammaxc*(wc+wk)/(wc(-1)+wk(-1))*exp(eta_wage)-1)*100;
hours_frbedo08_cql_obs   = (lc+lk)/(Lcstar+Lkstar);
...
varobs gdp_rgd_obs gdpdef_obs ffr_obs cnds_nom_obs cd_nom_obs ir_nom_obs inr_nom_obs cnds_def_obs cd_def_obs wage_rgd_obs hours_frbedo08_cql_obs;

estimation(nodisplay, smoother, order=1, prefilter=0, mode_check, bayesian_irf, datafile=data_20090210, xls_sheet=s4, xls_range=B1:AN101, presample=4, mh_replic=1000000, mh_nblocks=1, mh_jscale=0.3, mh_drop=0.3, sub_draws=5000, forecast=40, mode_compute=1) gdp_rgd_obs;