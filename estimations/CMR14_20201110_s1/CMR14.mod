...
@# define financial_data = ["networth_rgd_cmr14_obs", "credit_rgd_obs", "baag10_obs", "g10ffr_obs"]
...
@#define net_worth_in_financial_data = ("networth_rgd_cmr14_obs" in financial_data)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  %                   * (aptil_p * pi_p * log(pi(+1) / pitarget) + (1 / 4) * adytil_p * muzstar_p * (gdp_rgd_obs-gdp_rgd_obs_mean)/100);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  @# if cee == 0
  networth_rgd_cmr14_obs = n / n(-1) * muzstar / muzstar_p;
...
stderr networth_rgd_cmr14_obs, INV_GAMMA_PDF,  0.01,              5;
@# endif
@# endif
end;
...
@# endif
@# if net_worth_in_financial_data
stderr networth_rgd_cmr14_obs,     0.0174589738467390;
@# endif
@# endif
...
estimation(nodisplay, smoother, order=1, prefilter=0, mode_check, bayesian_irf, datafile=data_20201110, xls_sheet=s1, xls_range=B1:AY100, presample=4, mh_replic=100000, mh_nblocks=1, mh_jscale=0.3, mh_drop=0.3, sub_draws=5000, forecast=40, mode_compute=0, mode_file=CMR14_mode) gdp_rgd_obs gdpdef_obs;