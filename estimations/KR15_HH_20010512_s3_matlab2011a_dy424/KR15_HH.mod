...
var lambda, R, pic, c, w, phi_w, psi_w, L, k, k_bar, i, Q, R_e, r_k, u, 
    ...
    gdp_rgd_obs, c_rgd_obs, dlnI, hours_kr15_obs, wage_rgd_obs, gdpdef_obs, ffr_obs,
    ...
    inr_nom_obs, ir_nom_obs, mortgage_nom_obs, hp_nom_obs, mortffr_obs;
...
gdp_rgd_obs=100*(log(y/y(-1))+z);
c_rgd_obs=100*(log(c/c(-1))+z);
dlnI=100*(log((i+i_o)/(i(-1)+i_o(-1)))+z);
hours_kr15_obs=100*log(L)+L_adj;
wage_rgd_obs=100*(log(w/w(-1))+z);
gdpdef_obs=100*log(pic);
ffr_obs=100*(R-1);
//dlnD=100*(log(d/d(-1))+z)+D_adj;
//dlnDn=100*(log(d/d(-1))+z+log(pic))+D_adj;
//Spread=400*(R_d-R);
inr_nom_obs=100*(log(i/i(-1))+z);
ir_nom_obs=100*(log(i_o/i_o(-1))+z);
mortgage_nom_obs=100*(log(d_i/d_i(-1))+z+log(pic))+D_adj;
hp_nom_obs=100*(log(Q_o/Q_o(-1))+log(pic))+Q_o_adj;
mortffr_obs=100*(R_i-R);
end;
...
varobs gdp_rgd_obs, c_rgd_obs, inr_nom_obs, hours_kr15_obs, wage_rgd_obs, gdpdef_obs, ffr_obs, ir_nom_obs, mortgage_nom_obs, hp_nom_obs, mortffr_obs;
...
//estimation(mode_compute=0,datafile=data,mode_file=dssw_mode,first_obs=1,nobs=152,plot_priors=0,nodiagnostic,mh_replic=1000,mh_nblocks=4,mh_jscale=0.25,mh_drop=0.2);
//stoch_simul(order=1,irf=0) gdp_rgd_obs;
...
estimation(nograph, smoother, order=1, prefilter=0, datafile=data_20010512, xls_sheet=s3, xls_range=B1:AY101, presample=4, mh_replic=100000, mh_nblocks=1, mh_jscale=0.3, mh_drop=0.3, forecast=40, mode_compute=0, mode_file=KR15_HH_mode) gdp_rgd_obs;