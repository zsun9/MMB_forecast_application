...
var 
	...
	unr_cql_obs			${u}$       	   (long_name='Unemployment')
	;
...
unr_cql_obs =u + 100 * (clandaw-1)/csigl; % OK
end; 
...
emp_obs = cebar; % OK
unr_cql_obs = 100*(clandaw-1)/csigl; % OK
...
varobs   ffr_obs    gdpdef_obs   gdp_rgd_obs    c_rgd_obs     ifi_rgd_obs    wage_rgd_obs   emp_obs      unr_cql_obs;
...
estimation(nodisplay, smoother, order=1, prefilter=0, datafile=data_20200211, xls_sheet=s4, xls_range=B1:BE101, presample=4, mh_replic=500000, mh_nblocks=1, mh_jscale=0.3, mh_drop=0.3, sub_draws=5000, forecast=40, mode_compute=6) gdp_rgd_obs;