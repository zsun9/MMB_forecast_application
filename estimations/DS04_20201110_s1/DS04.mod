...
//**************************************************************************
// Common Variables                                                      //*    
   interest ffr_obs inflation gdpdef_obs outputgap gdp_rgd_obs;         //*
//**************************************************************************
...
gdp_rgd_obs = trend + (y-y(-1)+z)*100;                                      //*
gdpdef_obs = inflationqstar + pie*100;                                   //*  
ffr_obs  = rstar + inflationqstar + R*100;                             //*  
//**************************************************************************
...
estimation(nodisplay, smoother, order=1, prefilter=0, mode_check, bayesian_irf, datafile=data_20201110, xls_sheet=s1, xls_range=B1:AY100, presample=4, mh_replic=1000000, mh_nblocks=1, mh_jscale=0.3, mh_drop=0.3, sub_draws=5000, forecast=40, mode_compute=1) gdp_rgd_obs gdpdef_obs;